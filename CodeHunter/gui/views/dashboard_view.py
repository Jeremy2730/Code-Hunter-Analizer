...
def _scan_project(path):
    ...
    # ✅ Usa walk_python_files — fuente única de verdad para exclusiones
    for fpath in walk_python_files(path):
        stats["files"] += 1

        # Módulo = subcarpeta directa bajo el proyecto
        rel = os.path.relpath(os.path.dirname(fpath), path)
...
class DashboardView(ctk.CTkFrame):
    ...
    def _build_readme_section(self, parent):
        C = self.colors
        ctk.CTkLabel(parent, text="📋  Descripción del Proyecto",
    ...
    def _build_recent_findings(self, parent):
        C = self.colors
        ctk.CTkLabel(parent, text="Hallazgos recientes",
            font=ctk.CTkFont(size=14, weight="bold"), text_color=C["text_primary"],
    ...
    # ──────────────────────────────────────────────────────────────────────────
    # GENERACIÓN DE INFO (hilo secundario)
    # ──────────────────────────────────────────────────────────────────────────
    ...
    def _generate_project_info(self, path):
        ...
        self.after(0, lambda: self.readme_label.configure(
            text="🤖  Analizando código con IA...", text_color=self.colors["text_muted"]))
    ...
    def _export_pdf(self):
        ...
        save_path = filedialog.asksaveasfilename(
            ...
            initialfile="codehunter_report.pdf", title="Guardar reporte PDF",
        )
        if not save_path:
            return
        try:
            ...
            output_path = export_report_to_pdf(
                ...
                {"score": score, "critical": critical, "warnings": warnings, "findings": finds},
            )
