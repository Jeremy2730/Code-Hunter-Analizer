...
class SearchView(ctk.CTkFrame):
    ...
    def _build_search(self):
        ...
        ctk.CTkButton(search_bar,
            text="Buscar",
            font=ctk.CTkFont(size=13, weight="bold"), height=42, width=100,
            fg_color=C["accent"], hover_color="#1F6FEB", corner_radius=8,
    ...
    def _do_search(self):
        ...
        threading.Thread(target=self._search_files, args=(query,), daemon=True).start()
    ...
    def _search_files(self, query: str):
        ...
        try:
            for dirpath, _, files in os.walk(root):
                ...
                if any(p in ("__pycache__", ".venv", "venv", ".git") for p in parts):
                    continue
                for fname in files:
    ...
    def _render_results(self, query: str, results: list):
        ...
        for r in results:
            row = ctk.CTkFrame(self.results_frame, fg_color=C["bg_card"], corner_radius=8)
            row.pack(fill="x", pady=4)
            row.grid_columnconfigure(0, weight=1)

            ctk.CTkLabel(row,
                text=f"📄 {r['file']}  :  línea {r['line']}",
                font=ctk.CTkFont(size=11, weight="bold"), text_color=C["accent"], anchor="w",
